      ...
      <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
        <div>
          ...
          <p className="text-sm text-gray-400 flex items-center gap-1 mt-0.5">
            ...
            {new Date(order.createdAt).toLocaleString('ar-SA', { dateStyle: 'medium', timeStyle: 'short' })}
          </p>
        </div>
        <div className="flex items-center gap-2">
          ...
          {order.status === 'pending' && (
            <button
              ...
              className="flex items-center gap-1 text-sm text-red-500 border border-red-200 rounded-lg px-3 py-1.5 hover:bg-red-50 transition-colors"
            >
        ...
        <div className="md:col-span-2 space-y-4">
              ...
              {order.orderItems.map((item: any) => (
                <div key={item.id} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-400 mt-0.5">الكمية: {item.quantity} × {Number(item.unitPrice).toFixed(2)} ج.م</p>
                    {item.discount > 0 && (
                      <p className="text-xs text-green-600">-{Number(item.discount).toFixed(2)} ج.م خصم</p>
                    )}
                  </div>
                  <p className="font-bold text-gray-900 shrink-0">{Number(item.totalPrice).toFixed(2)} ج.م</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="card p-5">
            ...
            <div className="text-sm text-gray-600 space-y-1">
              <p><span className="font-medium text-gray-700">هاتف 1:</span> {order.shippingPhone1}</p>
              {order.shippingPhone2 && <p><span className="font-medium text-gray-700">هاتف 2:</span> {order.shippingPhone2}</p>}
              {order.shippingTitle && <p><span className="font-medium text-gray-700">العنوان:</span> {order.shippingTitle}</p>}
              {order.shippingDescription && <p className="text-gray-400">{order.shippingDescription}</p>}
              {order.notes && (
              ...
              <div className="border-t border-gray-100 pt-2 mt-2 space-y-1.5">
                ...
                {order.discountAmount > 0 && (
                  <div className="flex justify-between text-green-600">
                    ...
                    <span>-{Number(order.discountAmount).toFixed(2)} ج.م</span>
                  </div>
                )}
                <div className="flex justify-between text-gray-600">
                  <span>الشحن</span>
                  <span>{Number(order.shippingAmount) > 0 ? `${Number(order.shippingAmount).toFixed(2)} ج.م` : 'مجاني'}</span>
                </div>
